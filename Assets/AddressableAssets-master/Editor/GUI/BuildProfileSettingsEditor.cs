...
namespace AddressableAssets
{
    internal class ProfileSettingsEditor
    {
        ...
        public ProfileSettingsEditor(AddressableAssetSettings settings)
        {
        ...
        private void CreateTreeView(AddressableAssetSettings settings)
        {
            if (tree == null)
            {
                if (treeState == null)
                ...
                tree = new BuildProfileSettingsTreeView(treeState, mchs, settings, this);
            }
        }

        static public void ValueGUI(AddressableAssetSettings settings, string label, AddressableAssetSettings.ProfileSettings.ProfileValue val)
        {
            var names = settings.profileSettings.GetAllVariableNames().ToList();
            names.Sort();
            names.Add("<Custom>");
            string[] displayNames = names.ToArray();
            int currentIndex = val.m_custom ? (names.Count - 1) : names.IndexOf(settings.profileSettings.GetVariableNameFromId(val.m_value));
            EditorGUILayout.BeginHorizontal();
            ...
            if (newIndex != currentIndex)
            {
                ...
                {
                    val.m_custom = true;
                    val.m_value = "[" + displayNames[currentIndex] + "]";
                }
                else
                {
                    val.m_custom = false;
                    val.m_value = settings.profileSettings.GetVariableIdFromName(names[newIndex]);
                }
            }
            if (val.m_custom)
                val.m_value = EditorGUILayout.TextField(val.m_value);
            else
            {
    ...
    internal class BuildProfileSettingsTreeView : TreeView
    {
        ...
        public BuildProfileSettingsTreeView(TreeViewState state, MultiColumnHeaderState mchs, AddressableAssetSettings settings, ProfileSettingsEditor editor) : base(state, new MultiColumnHeader(mchs))
        {
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            foreach (var v in m_settings.profileSettings.GetAllVariableIds())
        ...
        private void CellGUI(Rect cellRect, TreeViewItem item, int column, ref RowGUIArgs args)
        {
            if (string.IsNullOrEmpty(item.displayName))
            {
                if ((SortOption)column == SortOption.Label)
                {
                    ...
                    {
                        if (m_settings.profileSettings.ValidateNewVariableName(name))
                        {
                            m_settings.profileSettings.SetValueByName(m_editor.editingProfile, name, string.Empty, true);
            ...
            var style = m_settings.profileSettings.IsValueInheritedById(m_editor.editingProfile, item.displayName) ? normalTextField : boldTextField;
            var displayName = m_settings.profileSettings.GetVariableNameFromId(item.displayName);
            switch ((SortOption)column)
            {
                case SortOption.Label:
                    ...
                    {
                        item.displayName = m_settings.profileSettings.GetVariableIdFromName(m_settings.profileSettings.RenameEntry(displayName, newName));
                        m_editor.dataNeedsReset = true;
                    }
                    break;
                case SortOption.Value:
                    var currVal = m_settings.profileSettings.GetValueById(m_editor.editingProfile, item.displayName);
                    var newVal = EditorGUI.DelayedTextField(cellRect, currVal, style);
                    if (newVal != currVal)
                    {
                        m_settings.profileSettings.SetValueById(m_editor.editingProfile, item.displayName, newVal);
                        m_editor.dataNeedsReset = true;
                    }
                    break;
                case SortOption.Evaluated:
                    EditorGUI.LabelField(cellRect, m_settings.profileSettings.Evaluate(m_editor.editingProfile, m_settings.profileSettings.GetValueById(m_editor.editingProfile, item.displayName)));
                    break;
            }
        }
        ...
        IOrderedEnumerable<TreeViewItem> InitialOrder(IEnumerable<TreeViewItem> myTypes, int[] columnList, bool ascending)
        {
            ...
            {
                case SortOption.Label:
                    orderFunc = (l => m_settings.profileSettings.GetVariableNameFromId(l.displayName));
                    break;
                case SortOption.Value:
                    orderFunc = (l => m_settings.profileSettings.GetValueById(m_editor.editingProfile, l.displayName));
                    break;
                case SortOption.Evaluated:
                    orderFunc = l => m_settings.profileSettings.Evaluate(m_editor.editingProfile, m_settings.profileSettings.GetValueById(m_editor.editingProfile, l.displayName));
                    break;
                default:
